...
slack = Slacker(token)
chrome_headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.95 Safari/537.36'}
firefox_headers = {'User-Agent': 'Mozilla/5.0 (Android 5.1; Tablet; rv:50.0) Gecko/50.0 Firefox/50.0'}
def pywget(url):

    # 파라미터들을 추출한다
    filename = url.split('/')[-1]
    r = requests.get(url, headers = firefox_headers, stream=True)
...
if __name__ == "__main__" :
    ...
    ThreadPool(4).imap_unordered(pywget, linklist)
