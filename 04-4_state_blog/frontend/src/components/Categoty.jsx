...
const Category = (props) => {
	...
	useEffect(()=> {
		...
		const fetchData = async() => {
			const urlink = `${process.env.REACT_APP_API_URL}/api/blog/category` 
			try{
				const res = await axios.post(urlink, {category}, config);
				setBlogs(res.data);
			}
			catch (error) {
				console.log(error);
	...
	return (
		<div className='container mt-3'>
			<h3 className="display-4">{currentCategory} Categoryv</h3>
