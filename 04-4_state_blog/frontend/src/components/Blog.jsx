...
const Blog = () => {
	...
	const [fecturedBlog, setFeaturedBlog] = useState([]);

	useEffect(()=>{ 
		const fetchData = async () => {
			const urlink = `${process.env.REACT_APP_API_URL}/api/blog/featured` 
			// const urlink = `${process.env.REACT_APP_API_URL}/api/blog`
			try {
		...
		<div className="container mt-3">
			...
			<div className="jumbotron p-4 p-md-5 text-white rounded bg-dark">
				<div className="col-md-6 px-0">
					<h1 className="display-4 font-italic">{fecturedBlog.title}</h1>
					<p className="lead my-3">{fecturedBlog.excerpt}</p>
					<p className="lead mb-0">
						<Link to={`/blog/${fecturedBlog.slug}`} className="text-white font-weight-bold">
							Continue reading...
						</Link>
					</p>
				</div>
